...
function jwppp_meta_box_callback( $post ) {
	...
	$clickyab_plugin_url = get_post_meta($post->ID, '_clickyab-plugin-url-1', true);
	$clickyab_vast_url_js = get_post_meta($post->ID, '_clickyab-vast-url-js-1', true);
	...
	echo '<style>';
	echo 'a.question-mark {position:relative; right:1rem;}';
	echo 'img.question-mark {position:relative; top:0.2rem;right:1em}';
	echo '</style>';

	echo '<label for="_jwppp-video-url-1">';
	echo '<strong>' . __( 'Media URL', 'jwppp' ) . '</strong>';
	echo '<a href="http://www.ilghera.com/support/topic/media-formats-supported/" title="More informations" target="_blank"><img class="question-mark" src="' . plugins_url('jwppp') . '/images/question-mark.png" /></a></th>';
	echo '</label> ';
...
//	echo '<p><input type="text" id="_jwppp-video-image-1" name="_jwppp-video-image-1" placeholder="' . __('Add a different poster image for this video', 'jwppp') . '" size="60" disabled="disabled" /></p>';
	...
	echo '<a class="question-mark" title="' . __('Only required when a file extension is missing or not recognized', 'jwppp') . '"><img class="question-mark" src="' . plugins_url('jwppp') . '/images/question-mark.png" /></a></th>';
	echo '</label>';
...
//	echo ' />';
//	echo '<strong>' . __('Add video chapters/ subtitles', 'jwppp') . '</strong> | <a href="http://www.ilghera.com/product/jw-player-7-for-wordpress-premium/" target="_blank">Upgrade</a>';
//	echo '</label>';
...
function jwppp_save_meta_box_data( $post_id ) {
	...
	if ( ! isset( $_POST['_clickyab-vast-url-js-1'] ) ) {
		return;
	}

	if ( ! isset( $_POST['_clickyab-plugin-url-1'] ) ) {
		return;
	}
	...
	$clickyabJS = sanitize_text_field($_POST['_jwppp-clickyab-plugin-url-1']);
	$clickyabVastUrl = sanitize_text_field($_POST['_jwppp-clickyab-vast-url-js-1']);
	...
	update_post_meta( $post_id, '_jwppp-clickyab-plugin-url-1', $clickyabJS );
	update_post_meta( $post_id, '_jwppp-clickyab-vast-url-js-1', $clickyabVastUrl );
...
function jwppp_video_code($p) {
		...
		$clickyabJS = sanitize_text_field(get_option('jwppp-clickyab-vast-url-js'));
		$clickyabVastUrl = sanitize_text_field(get_option('jwppp-clickyab-plugin-url'));
		...
		$output .= "<script type='text/javascript'>\n";
			...
			$output .= "playerInstance_$this_video.setup({\n";
				...
				if($clickyabJS) {
					...
					$output .= "advertising: {\n";
					$output .= "client: \"vast\",\n";
					$output .= 	"schedule: '" . $clickyabVastUrl . "'\n";
					$output .= "},\n";
				}

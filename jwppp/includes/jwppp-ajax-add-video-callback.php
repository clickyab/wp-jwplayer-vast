...
function jwppp_ajax_add_video_callback($post) {
    ...
    $clickyab_plugin_url = get_post_meta($post->ID, '_jwppp-clickyab-plugin-url', true);
    $clickyab_plugin_url = get_post_meta($post->ID, '_jwppp-clickyab-plugin-url', true);
	...
	echo '</td>';
	if($number<2) {
		echo '<td class="add-video"><a class="jwppp-add"><img src="' . plugins_url('jwppp') . '/images/add-video.png" /></a></td>';
	} else {
		echo '<td class="remove-video"><a class="jwppp-remove" data-numb="' . $number . '"><img src="' . plugins_url('jwppp') . '/images/remove-video.png" /></a></td>';
	}
